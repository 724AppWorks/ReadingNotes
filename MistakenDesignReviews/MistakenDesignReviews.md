...
## レビュー技法
レビュー技法には様々な種類があります。
経営層や管理職がプロジェクトの状況把握や承認のために実施する「マネジメントレビュー」。
法令や国際標準規格、社内規則、開発基準に準拠しているかを調べる「オーディット」
エンジニアが行うレビューは「ピアレビュー」と呼ばれます。
「ウォークスルー」や「テクニカルレビュー」もピアレビューの一種です。
ピアレビューにはその他にも「インスペクション」というレビューもあります。
ピアレビューは、出席者が対等な関係で行うレビューです。（役職とかそういう話ではなく）

### ウォークスルー
ウォークスルーは、ドキュメント作成者が主体で開催する、カジュアルなレビューです。
ウォークスルーの方法は、ドキュンメント作成者が設計における悩み事相談し、レビューアが答えるものです。
ドキュメントは完成していなくても構いませんし、ドキュメント内についての指摘だけに留める必要もありません。
ウォークスルーのメリットは、作成者の悩み事を解消し、早い段階でチームの認識を合わせることにあります。
デメリットは、話題が膨らみやすいので、会議が長引きやすいことです。ある程度時間は区切っておきましょう。

### インスペクション
インスペクションはウォークスルーと対極にある、フォーマルなレビューです。
予め決められたルールに沿って進行されます。（観点、完了条件、収集指標など）
網羅的に問題検出することが目的であり、メリットです。
参加者にも役割が定義されます。記録係、読み上げ係とか、プログラマの役割とか。
インスペクションは、進行役が主導します。
デメリットは、準備が大変ということです。
役割定義を明確にすることで、検出内容もイメージしやすくなるかもです。

### テクニカルレビュー
テクニカルレビューは、一般的に「レビュー」と呼ばれるような、技術リーダー主導のチェックです。
テクニカルレビューの主な目的は、ドキュメントの問題検出です。
インスペクションと同様に、事前にドキュメントの問題検出をした上での実施が推奨されます。
インスペクションと違うのは、ルール決めと遵守の必要はないということです。
ただし、シナリオのように問題検出の指針を決めておくのは有効です。
テクニカルレビューのメリットは、インスペクションほど形式ばらず、かつウォークスルーより自由度が高いため、
一定の成果を上げやすいことです。
デメリットは、自由度が高くなるために、進行役がレビューアの成り行きに任せてしまい、脱線することがある点です。

### Tips レビュー技法は組み合わせて利用する
1つのレビュー技法だけを採用するのではなく、状況や必要な成果に合わせて複数のレビューを組み合わせるとより効果的です。

# 第4章 さらなる効果向上
## レビューの改善・効率化
プロジェクトが完了したとき、プロジェクトの振り返りを行うのと同様に、レビューの振り返りも行いましょう。
レビューの振り返りを行う目的は、レビュー方法の改善と、問題発生の防止です。

### レビューの改善
レビューの改善は、「後工程で見つかった不具合の洗い出し」「作成時の予防方法の検討」
「シナリオの見直し・新規作成」「シナリオの削減」「マインドの見直し」のプロセスで構成されます。

#### 後工程で見つかった不具合の洗い出し
後工程で見つかった不具合のうち、レビューで検出できた問題を洗い出します。

#### 作成時の予防方法の検討
重大な問題を検出方法を確認して、ドキュメント作成時に予防できる方法がないかを確認します。
内容の詳細度であったり、より理解しやすい表記方法がないか検討するなどします。

#### シナリオの見直し・新規作成
シナリオをベースに、レビューのやり方を見直します。
洗い出しを行った重大な問題について、シナリオの通りに実施すれば防げていたかを確認します。
既存のシナリオでの検出が難しい場合は、新たなシナリオを検討します。
既存のシナリオで検出できていた場合では、既存のシナリオを見直します。
ちゃんとやっていれば検出できたのに。。。とか言っちゃうのはあまりよくないです。
確実に検出できるようなシナリオになるよう、表現方法などで誤解を招くことがないかなどを確認、修正していきます。

#### シナリオの削減
多すぎるシナリオは、レビューの負荷に直結します。
振り返った結果として、重大な問題検出が期待できないと判断したシナリオは削除を検討しましょう。
シナリオ内のチェック項目、チェック方法についても、不要だと思われるものがあれば削除検討をしましょう。

#### マインドの見直し
リーダーを中心に、レビューを行う上でのマインドの見直しを行います。
作成者へ向けての批判、厳しい指摘などがないかを確認します。
ただ、特定の個人に対してではなく、一般論として話を進めなければ批判と変わらないので、注意して進めましょう。

## 開発タイプ別のレビュー手順
レビューの方法は、開発タイプによって変わってきます。
ここでは、「保守開発」「反復開発」「アジャイル開発」「パッケージ・クラウド開発」「大規模開発」について取り上げます。
レビューのプロセスは前述していますが、
「レビューの準備」「問題の検出」「問題の指摘」「修正・フォロー」で構成されます。

### 保守開発
保守開発は、従来のバージョンとの差分を利用できることが特徴です。

#### レビューの準備
保守開発では、既存のシナリオや、レビュー履歴があるはずなので、
そこから今回必要なシナリオなどを選定します。
ドキュメントとしても、既存との差分が確認できるような資料があると、レビューしやすくなります。

#### 問題の検出
割り当てられたシナリオに沿って、問題の検出を行います。
ドキュメントがバージョン管理されていることもあるので、既存バージョンとの差分チェックは後回しにしましょう。
途中で修正が入るかもしれないので。

#### 問題の指摘
レビュー会議で指摘をしていきます。
保守開発だと、既存ソースについての指摘（そもそもこれってなんで…）が出ることがあります。
そのため、既存ソースについての知見があるメンバーにも参加してもらうのも手です。

#### 修正・フォロー
指摘された問題について、設計者は修正を行います。
レビューアは修正内容の既存との整合性をチェックしつつフォローします。

## 反復開発
反復開発は、1回転の周期が短いことが多い（というか強み）のため、
1回のレビューにあまり多くの時間がかけられないことがあります。
レビュー時間を短くするポイントは2つです。

### シナリオを減らす
効果の低いシナリオを減らしたり、ドキュメントを絞ったりして、レビュー負荷を下げます。

### 再発防止を生かす
レビューで検出される問題が少なくなれば、負荷は減ります。
再発防止策を講じ、イテレーションの中で磨きをかけることで、レビューの負荷を下げます。

## アジャイル開発
基本は反復開発と同様です。
ただ、アジャイル開発としては、レビュー会議を別の方法で代替します。
例えばペアプログラミングや、テストファースト、継続的インテグレーションなどです。
日次のミーティングが、小さなレビューの役割を果たせるようにするなども効果的です。
アジャイルでは、ドキュメントの作成より動作するプログラムを重視するため、
ソースコードの可読性に視点を合わせるようにしましょう。

## パッケージ・クラウド開発
ブラックボックスなので、動かしてみないとわからない箇所もありますが、
IF仕様などを読み解いていけば、問題種別を検討できるので、レビューを行います。
要件定義書、仕様書、設計書のドキュメントレビューを実施します。
スケーラビリティや、ライセンス形態についても、
スループットや同時アクセス数など、問題種別を検討しましょう。

## 大規模開発
大規模開発ではサブシステムごとにチームが作成されることが多いです。
レビューはサブシステム、つまりチームごとに行うようにします。
また、サブシステム横断のレビューなども実施します。

### レビューの準備
アーキテクチャの検討などから、検出すべき問題種別を検討します。
サブシステム間の連携も必要なので、サブシステムのリーダーが集まるようなレビューも確保します。

### レビュー負荷を下げる
大規模開発では、重大な問題が発覚した場合の修正工数が大きくなりがちです。
それに伴い、レビューの負担が大きくなります。
そのため、ドキュメントの記述方法をサブシステム間で統一するなどして、
サブシステム横断で行うレビューでの問題種別の種類を整理しておきます。

# レビュー観点絞り込みの効果
重要な問題を検出することが、工数の低減に繋がります。
レビュー観点を重要な問題を検出できるシナリオに絞り込めば、
シナリオにかける時間が増え、重大な問題の検出数も増えます。
他のシナリオにかける時間が減り、重大な問題が検出されたことで修正工数が減ります。
結果として全体の工数がマイナスになりやすいという考え方です。
